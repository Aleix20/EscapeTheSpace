...
float Vector2::perpdot( const Vector2& v ) const
{
	return y * v.x + -x * v.y;
}
...
Vector2 operator - (const Vector2& a, const Vector2& b) { return Vector2(a.x - b.x, a.y - b.y); }
bool operator == (const Vector2& a, const Vector2& b) { return (abs(a.x - b.x) + abs(a.y - b.y)) < 0.0001; }

Vector2 rotate(const Vector2& a, float degs) {
	float angle = degs*DEG2RAD;
	float sina = sin(angle);
	float cosa = cos(angle);
	return Vector2(a.x * cosa - a.y * sina, a.x * sina + a.y * cosa);
}
...
Vector3 rotateY(const Vector3& a, float degs) {
	...
	float sina = sin(angle);
	float cosa = cos(angle);
	return Vector3(a.x * cosa - a.z * sina, a.y, a.x * sina + a.z * cosa);
}
...
{
	double d = 1.0 / n.length();
	return Vector3(n.x * d, n.y * d, n.z * d);
}
...
{
	Quaternion Q;
	Q.setAxisAngle(axis, degs * DEG2RAD );
	return Q.rotate(a);
}
...
{
	return a - n * 2.f * dot(a, n);
}

//returns the angle between both vectors in radians
...
Vector3 RayPlaneCollision(const Vector3& plane_pos, const Vector3& plane_normal, const Vector3& ray_origin, const Vector3& ray_dir)
{
	...
	double numer = plane_normal.dot(ray_origin) - D; //I changed this
