...
Game::Game(int window_width, int window_height, SDL_Window* window)
{
	...
	stageIntro = new StageIntro();
	stageMenu = new StageMenu();
	stageOutro = new StageOutro();
	stageGameOver = new StageGameOver();
	
	//currentStage = stageIntro;

	enableAudio(); //enable this line if you plan to add audio to your application
	world.currentSound = synth.playSample("data/sounds/Interestellar.wav", 0.5, true);
}
...
void Game::render(void)
{
	...
	if (currentStage == eStages::STAGEMENU) {
		currentStageG = stageMenu;
	}
	else if (currentStage==eStages::STAGEINTRO) {
		currentStageG = stageIntro;
	}
	else if(currentStage == eStages::STAGEPLAY) {
		if (gameSound) {
			gameSound = false;
			world.currentSound = synth.playSample("data/sounds/interGame.wav", 0.5, false);
		}
		currentStageG = stagePlay;
	}
	else if (currentStage == eStages::STAGEGAMEOVER) {
		world.currentSound->stop();
		currentStageG = stageGameOver;
		
	}
	else {
		currentStageG = stageOutro;
		
	}
	currentStageG->render(&framebuffer, &world, &time);
...
{
	currentStageG->update(seconds_elapsed, &world);
